...
/**
 * @file
 * Contains \DynamicExampleMigration.
 */

abstract class DynamicExampleMigrateBase extends Migration {
  ...
  public function __construct($arguments) {
    ...
    if (!in_array($this->entityType, array('node', 'taxonomy_term'))) {
      throw new Exception('\DynamicExampleMigration supports only nodes and terms.');
    }
  ...
  protected function getMigrateDirectory() {
    return variable_get('dynamic_example_migrate_directory', FALSE) ? variable_get('dynamic_example_migrate_directory') : drupal_get_path('module', 'dynamic_example_migrate');
  }
}
